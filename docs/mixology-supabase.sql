...
create table if not exists public.mixology_items (
  ...
  kind text not null check (kind in ('character', 'persona', 'preface', 'base', 'flavor', 'glass', 'strength', 'ticket', 'garnish', 'encore', 'checklist', 'filter', 'mechanism')),
  name text not null,
...
--  1) kind 检查约束补齐十三类：persona（面具）、preface（序言）、checklist（核对）、filter（滤网）、mechanism（机括）
...
alter table public.mixology_items
  ...
  check (kind in ('character', 'persona', 'preface', 'base', 'flavor', 'glass', 'strength', 'ticket', 'garnish', 'encore', 'checklist', 'filter', 'mechanism'));
